...
const taskSchema = new mongoose.Schema({
  ...
  repeatDatesOfMonth: [{ type: Number, min: 1, max: 31 }],  // Multi-select dates
  creditPoints: { type: Number, default: 0 },
  priority: { type: String, enum: ['low', 'medium', 'high', 'urgent'], default: 'medium' },
  nextFollowUpDateTime: { type: Date },  // For non-repeat
  nextFinishDateTime: { type: Date },  // End recurrence datetime
  images: [{ type: String }],  
  audios: [{ type: String }],  
  files: [{ type: String }], 
