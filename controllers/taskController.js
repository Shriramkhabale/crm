  ...
  try {
    ...
    const images = req.files?.images
      ? req.files.images.map((f) => {
          console.log('📷 Image file from multer:', { 
          ...
          return {
            ...
            name: f.originalname || f.filename || 'image',
    ...
    const audios = req.files?.audios
      ? req.files.audios.map((f) => {
          console.log('🎵 Audio file from multer:', { 
            originalname: f.originalname, 
            filename: f.filename,
            path: f.path, 
            secure_url: f.secure_url 
          ...
          return {
            url: f.path || f.secure_url,
            name: f.originalname || f.filename || 'audio',
...
exports.updateTask = async (req, res) => {
  try {
    ...
    const normalizeAttachment = (item, fallbackType) => {
      if (!item) return null;
      ...
      if (typeof item === "object") {
        const url = item.url || item.path || item.secure_url;
        if (!url) return null;
        const name =
          item.name ||
          item.filename ||
    ...
    const mergeAttachments = (current, existingUrls, removedUrls, uploadedFiles, fallbackType) => {
      const removedSet = new Set(removedUrls || []);
      const existingSet = new Set(existingUrls || []);
      const byUrl = new Map();
      (current || [])
        .map((item) => normalizeAttachment(item, fallbackType))
        .filter((att) => att && att.url && !removedSet.has(att.url))
        .forEach((att) => {
          if (existingSet.size === 0 || existingSet.has(att.url)) {
            byUrl.set(att.url, att);
          }
        });
      (existingUrls || []).forEach((url) => {
        if (!url || removedSet.has(url)) return;
        if (!byUrl.has(url)) {
          const att = normalizeAttachment(url, fallbackType);
          if (att && att.url) {
            byUrl.set(att.url, att);
          }
        }
      ...
      if (uploadedFiles && Array.isArray(uploadedFiles)) {
        uploadedFiles.forEach((file) => {
          const att = normalizeAttachment(file, fallbackType);
          if (att && att.url) {
            byUrl.set(att.url, att);
          }
        });
